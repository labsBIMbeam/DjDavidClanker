/**
 * The bit that actually performs. Ported from Zazawowow's PR #8 and adapted
 * to this engine (syncTo/setSynced, transition-engine automix) — the mood
 * system and the gesture/undo design are his.
 *
...
export class Performer {
   ...
   * @param {{automix?: object, onStatus?: (s: string) => void, onCrossfade?: (v: number) => void}} [hooks]
   */
  ...
  /** The automix owns the crossfader while a handover runs (either engine). */
  _faderBusy() {
    return Boolean(this.automix && (this.automix.fade || this.automix.transition));
  }
  ...
  _align() {
    ...
    if (!other.syncTo(live)) return false; // unreachable — leave it be
  ...
  _blend(bars) {
    ...
    if (this._faderBusy()) return; // the fader is spoken for
