 ...
 * accurate), while record motion is pushed at tick rate into the granular
 * Turntable. Those two paths have different latencies — see LATENCY.
 */
...
export class AutoScratch {
  ...
  stop() {
    ...
    if (d._turntable) {
      ...
      d._turntable.setPosition(this.anchor);
    }
  ...
  tick() {
    ...
    let disp = move.disp0 + move.v * shapeDisp(move.shape, u) * move.dur * this.intensity;
