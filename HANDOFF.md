...
As of: August 15, 2026 · Status: **working, 242 E2E checks across 9 suites + 13 pytest green**
...
the Wavlake suites), `playlist-check` (23), `local-check` (9) and the
`zaza-check` visual probe — 242 JS checks, plus `uv run pytest` in
`ingest/` (13, including a real ffmpeg loudnorm run and the /proxy
guards with one live relay fetch).
...
| ≡ QUEUE desk | third browser mode: the WHOLE running queue pinned above the source list (queue + playlist in one view), order mode (LIST/SHUF/SMART) switchable in its header, source tabs keep working below. Rows in this mode carry `+` (append, deduped via `automix.append`), `1 2 3` (`automix.insertAt` — inserts or MOVES into the slot, counts as materialized so shuffle/smart keep the pin) and the desk header's `+ ALL FROM LIST` appends additively — the answer to "⇩ LIST replaced my queue from the wrong tab" from the first live night |
